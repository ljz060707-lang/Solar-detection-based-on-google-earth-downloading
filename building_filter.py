...
XMIN, YMIN = 18.5925, -33.9415
XMAX, YMAX = 18.6015, -33.9325
...
def download_osm_buildings() -> gpd.GeoDataFrame:
    ...
    try:
        import osmnx as ox
        # 设置 osmnx 全局配置（增加超时、换API）
        ox.settings.timeout = 300
        ox.settings.overpass_endpoint = "https://overpass-api.de/api/interpreter"
        # 备用API（如果上面的不行，换这个）："https://z.overpass-api.de/api/interpreter"

        print(f"[OSM] 正在下载建筑轮廓...")
        print(f"  查询范围：北(YMAX)={YMAX}, 南(YMIN)={YMIN}, 东(XMAX)={XMAX}, 西(XMIN)={XMIN}")

        # ✅ 核心修正：bbox 顺序 = (north, south, east, west)
        ...
        print(f"[OSM] 下载失败: {str(e)[:200]}")  # 打印具体错误（而非卡死）
        return gpd.GeoDataFrame(columns=["geometry", "source"], crs="EPSG:4326")
